...
class Task(models.Model):
    ...
    model_target = fields.Many2one('ir.model', string='Target object', ondelete='set default',
                                   required=True)
    ...
    def import_run(self, ids=None):
        ...
        if self._name == 'base.external.import.task':
            run_ids = self.ids
        ...
        for action_ref in actions:
            ...
            # now() microseconds are strippeddelete from product_product where cr
            ...
            params = [(sync)]
            exec_result = db_model.execute(obj.sql_source, params, metadata=True)
            rows = exec_result['rows']
            cols = exec_result['cols']
            _logger.error("MAB - Rows: %s" % (rows,))
            _logger.error("MAB - Cols: %s" % (cols,))

            # Exclude columns titled "Id"; add (xml_)"id" column
            #cidx = ([i for i, x in enumerate(cols)
            #         if x.upper() != 'ID'])
            #cols = ([x for i, x in enumerate(cols)
            #         if x.upper() != 'ID'] + ['id'])
            cols = ([x for x in list(cols)
                     if x.upper() != 'ID']) # + ['id'])
            cols2 = cols + ['id']
            ...
            for row in rows:
                ...
                for i in cols:
                    ...
                    if isinstance(v, str):
                        v = v.strip()
                    data.append(v)
                data.append(build_xmlid(row['id']).strip())

                # Import the row; on error, write line to the log
                log['last_record_count'] += 1
                self._import_data(cols2, data, model_obj, obj, log)
