...
class AccountInvoiceElectronic(models.Model):
    ...
    def _compute_invoice_amount_text(self):
        for inv in self:
            inv.invoice_amount_text = inv.currency_id and inv.currency_id.amount_to_text(round(inv.amount_total,2)).upper()
    ...
    def _compute_economic_activities(self):
        for inv in self:
            _logger.error('COMPANY  E C O N O M I C    I D S: %s', inv.company_id.economic_activities_ids)
            _logger.error('COMPANY  E C O N O M I C    I D: %s', inv.company_id.activity_id)
            inv.economic_activities_ids = inv.company_id.economic_activities_ids # self.env['economic.activity'].search([('active', '=', False)])
            if not inv.economic_activity_id:
                inv.economic_activity_id = inv.company_id.activity_id #.id

    @api.onchange('partner_id','tipo_documento')
    def _compute_partner_economic_activities(self):
        _logger.error('O N    C H A N GE    PARTNER  ECONOMIC ACTIVITIES! !')
        for inv in self:
            if inv.move_type in ('in_invoice', 'in_refund') and inv.tipo_documento == 'FEC':
                if inv.partner_id:
                    _logger.error('PARTNER  E C O N O M I C    I D S: %s', inv.partner_id.economic_activities_ids)
                    _logger.error('PARTNER  E C O N O M I C    I D: %s', inv.partner_id.activity_id)
                    inv.partner_economic_activities_ids = inv.partner_id.economic_activities_ids
                    #if not inv.partner_economic_activity_id:
                    inv.partner_economic_activity_id = inv.partner_id.activity_id
                else:
                    _logger.error('FEC  no partner !!')
    ...
    def _onchange_partner_id(self):
        _logger.error('O N    C H A N GE    PARTNER  ID! !')
                                ...
                                for i in line_taxes['taxes']:
                                    if taxes_lookup[i['id']]['tax_code'] == 'service':
                                        total_servicio_salon += round(
                                            subtotal_line2 * taxes_lookup[i['id']]['tarifa'] / 100, 5)

                                    elif taxes_lookup[i['id']]['tax_code'] != '00':
                                        tax_index += 1
                                        tax_amount = round(subtotal_line2 * taxes_lookup[i['id']]['tarifa'] / 100, 5)
