...
class CompanyElectronic(models.Model):
    ...
    activity_id = fields.Many2one(related='partner_id.activity_id',
    ...
    def action_get_economic_activities(self):
        if self.vat:
            ...
            _logger.error('E-INV CR  - Economic Activities JSON: %s', json_response)
            if json_response["status"] == 200:
                ...
                _logger.error('E-INV CR  - Activities: %s', activities)
                _logger.error('E-INV CR  - Activity codes: %s', a_codes)
                ...
                _logger.error('E-INV CR  - Economic Activities: %s', economic_activities)
            ...
            return {'value': {'vat': ''}, 'warning': alert}
        """
        if self.vat and self.partner_id:
            self.partner_id.action_get_economic_activities()
