...
class PurchaseOrderLine(models.Model):
    ...
    budget_line_ids = fields.One2many('crossovered.budget.line', compute='_compute_budget_line_ids', string='Budget Lines')
            ...
            if line.analytic_json and line.product_qty - line.qty_received > 0:
                for json in line.analytic_json:
                    return tuple([
                        ...
                        if key in self.env['crossovered.budget.line']._fields
                    ])

        for domain, lines in self.grouped(get_domain).items():
            lines.budget_line_ids = bool(domain) and self.sudo().env['crossovered.budget.line'].search(list(domain))
