...
class InvoiceLineElectronic(models.Model):
    ...
    total_tax = fields.Float(compute="_compute_line_total_tax")
    ...
    def _compute_line_total_tax(self):
        for record in self:
           line_tax_amount = 0
           line_tax_total = 0
           line_tax_price_include = False
           for tax in record.tax_ids:
              new_tax = self.env['account.tax'].search([('tax_code', '=', tax.tax_code),
                             ('amount', '=', tax.amount),
                             ('price_include', '=', tax.price_include),
                             ('active', '=', True)], limit=1)
              line_tax_amount = new_tax.amount
              line_tax_price_include = new_tax.price_include
           if line_tax_price_include:
              line_tax_total = (record.price_unit * record.quantity)-(record.price_unit * record.quantity) / ( 1 + line_tax_amount / 100 )
           else:
              line_tax_total = (record.price_unit * record.quantity) * line_tax_amount / 100
           record.total_tax = round(line_tax_total,2)
